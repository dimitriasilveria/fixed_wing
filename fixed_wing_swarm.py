...
for j in range(3):
    ...
    phi,target_r, target_v,target_a, _, _ = embedding.targets(target_r, target_v, phi, False)

for i in range(n_agents):
    controllers[i].references(0,target_r[:,i], target_v[:,i], target_a[:,i])
    controllers[i].set_initial_conditions(0)

phi,target_r, target_v, target_a, phi_diff_new, _ = embedding.targets(target_r,target_v,phi,False)
